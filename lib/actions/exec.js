...
async function processAction(msg, cfg) {
  try {
    ...
    const { rule } = cfg;
    // const { flowId, flowExecId } = msg.properties.headers;
    const { apiKey, flowId, flowExecId } = jwt.decode(msg.properties.headers.orchestratorToken);

    const snapshotData = await getSnapshotDataForFlow({ flowId, iamToken: apiKey });
